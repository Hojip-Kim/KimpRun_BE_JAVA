...
public class BithumbWebsocketClient extends WebSocketClient {
    ...
    public BithumbWebsocketClient(String serverUri, BithumbWebsocketHandler bithumbWebsocketHandler, Bithumb bithumb) throws URISyntaxException {
    ...
    public void onOpen(ServerHandshake handshakedata) {
        ...
        try {
            ...
            List<String> codes = bithumb.getMarketList().getKrCryptoNameList();
    ...
    public void onMessage(ByteBuffer message) {
        ...
        if(!"{\"status\":\"UP\"}".equals(receivedString)) {
            try {
                BithumbReceiveDto bithumbReceiveDto = objectMapper.readValue(receivedString, BithumbReceiveDto.class);
                ...
                bithumbWebsocketHandler.inputDataToHashMap(bithumbDto);
    ...
    public void onError(Exception ex) {
        log.error("[빗썸] 웹소켓 에러 : " + ex.getMessage(), ex);
        isConnected = false;
        attemptReConnect();
    }
        ...
        reconnectTask = executor.submit(() -> {
            try {
                while (!isConnected) {
                    try {
                        Thread.sleep(5000);
                        log.info("[빗썸] 웹소켓 재연결 시도");
                        this.reconnectBlocking();
                        if (this.isOpen() && checkNetworkConnect()) {
                            isConnected = true;
                            log.info("[빗썸] 웹소켓 재연결 성공");
                        } else {
                            log.warn("[빗썸] 연결은 성공하였으나, 네트워크 상태 불안정. 네트워크 연결 다시 시도");
                            isConnected = false;
                        }
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt(); // 스레드의 인터럽트 상태를 유지하며 스레드를 종료
                        log.error("[빗썸] 웹소켓 재 연결 방해됨.", e);
                        break;
                    } catch (Exception e) {
                        log.error("[빗썸] 웹소켓 재연결 실패", e);
                    }
                }
            } catch (Exception ex) {
                log.error("[빗썸] 웹소켓 재연결 중 문제 발생", ex);
            } finally {
                if (isConnected) {
    ...
    private boolean checkNetworkConnect() {
        try{
            ...
            log.error("[빗썸] 네트워크 상태 확인 중 방해됨", e);
            Thread.currentThread().interrupt();
            return false;
        }catch(Exception e){
            log.error("[빗썸] 네트워크 상태 확인 중 오류 발생", e);
            return false;
        }
    }
    ...
    public void sendPingMessage(){
        if (isConnected){
            try{
                this.send("PING");
                log.info("[빗썸] 연결 수립을 위한 PING 메시지 송신");
            }catch(Exception e){
                log.error("[빗썸] PING 메시지 송신 실패", e);
            }
        }else{
            log.warn("[빗썸] 웹소켓 서버 연결 안됨");
        }
    }
}
